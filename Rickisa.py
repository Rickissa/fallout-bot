...
async def settlement_income_callback(context: ContextTypes.DEFAULT_TYPE):
    ...
    add_log(context, f"Доход поселения {player_id} (индекс {set_idx}): {daily_income} крышек")
...
def cancel_settlement_income(job_queue: JobQueue, player_id: str, set_idx: int = -1) -> None:
    if job_queue is None:
        return
    if set_idx == -1:
        # Отменяем все поселения для этого игрока
        current_jobs = [job for job in job_queue.jobs() if job.name.startswith(f"settlement_{player_id}_")]
        for job in current_jobs:
            job.schedule_removal()
    else:
...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data == "actions_add":
            ...
            await ctx.bot.send_message(chat_id=int(uid), text="Введи за��ись о ходе действий:")
            return
        ...
        if data == "player_settlement":
            ...
            txt = f"🏘 МОИ ПОСЕЛЕНИЯ\n\n"
            txt += f"Всего: {len(settlements)}\n\n"
            
            if settlements:
                for i, s in enumerate(settlements, 1):
                    txt += f"{i}. {s.get('name')} | Люди: {s.get('population', 0)} | Доход: {s.get('daily_income', 0)}\n"
            else:
                txt += "Поселений нет"
            
            kb = []
            for i, s in enumerate(settlements):
                name = s.get('name', f'Поселение {i+1}')
                kb.append([InlineKeyboardButton(f"🏘 {name}", callback_data=f"player_set_manage_{i}")])
            kb.append([InlineKeyboardButton("Назад", callback_data="player_profile")])
            
            await ctx.bot.send_message(chat_id=int(uid), text=txt, reply_markup=InlineKeyboardMarkup(kb))
            return

        if data.startswith("player_set_manage_"):
            try:
                set_idx = int(data.replace("player_set_manage_", ""))
            except:
            ...
            ctx.user_data["current_settlement_idx"] = set_idx
            
            kb = [
                [InlineKeyboardButton("💰 Снять крышки", callback_data="player_set_withdraw")],
                [InlineKeyboardButton("📥 Добавить крышки", callback_data="player_set_deposit")],
                [InlineKeyboardButton("Назад", callback_data="player_settlement")]
            ]
            await ctx.bot.send_message(chat_id=int(uid), text=txt, reply_markup=InlineKeyboardMarkup(kb))
            return

        if data == "player_set_withdraw":
            player = players.get(uid)
            if not player:
                return
            settlements = player.get("settlements", [])
        ...
        if data == "player_profile":
            ...
            txt += f"Поселений: {len(settlements)}"
        ...
        logger.exception(f"Ошибка в callback_handler: {e}")
        await ctx.bot.send_message(chat_id=int(uid), text=f"⚠️ Ошибка: {str(e)[:100]}")
        return
    ...
    await ctx.bot.send_message(chat_id=int(uid), text="Неизвестная кнопка или функция в разработке.")
        ...
        if data.startswith("adm_player_"):
            if admin_id != user.id:
                ...
                return
            pid_str = data.replace("adm_player_", "")
            ...
            kb = [
                ...
                [InlineKeyboardButton("🏘 Поселения", callback_data=f"adm_settlements")],
        ...
        if data.startswith("adm_set_player_"):
            if admin_id != user.id:
                await ctx.bot.send_message(chat_id=int(uid), text="Только админ.")
                return
            pid_str = data.replace("adm_set_player_", "")
        ...
        if data.startswith("adm_set_manage_"):
            ...
            # Правильный парсинг: adm_set_manage_PID_IDX
            parts = data.split("_")
            if len(parts) < 5:
        ...
        if data.startswith("adm_set_create_"):
            if admin_id != user.id:
                return
            pid_str = data.replace("adm_set_create_", "")
        ...
        if data.startswith("adm_set_remove_balance_"):
            ...
            await ctx.bot.send_message(chat_id=int(uid), text="Введите количество крешек для вычитания из баланса поселения:")
            return
        ...
        if data.startswith("adm_sched_pay_player_"):
            if admin_id != user.id:
                await ctx.bot.send_message(chat_id=int(uid), text="Только админ.")
                return
            pid_str = data.replace("adm_sched_pay_player_", "")
            kb = [
        ...
        if data.startswith("adm_sched_pay_caps_"):
            if admin_id != user.id:
                return
            pid_str = data.replace("adm_sched_pay_caps_", "")
        ...
        if data.startswith("adm_sched_pay_goods_"):
            if admin_id != user.id:
                return
            pid_str = data.replace("adm_sched_pay_goods_", "")
        ...
        if data.startswith("adm_sched_cancel_caps_"):
            if admin_id != user.id:
                return
            pid_str = data.replace("adm_sched_cancel_caps_", "")
        ...
        if data.startswith("adm_sched_cancel_goods_"):
            if admin_id != user.id:
                return
            pid_str = data.replace("adm_sched_cancel_goods_", "")
        ...
        # ===== ADMIN: OTHER OPERATIONS (остальное из старого кода) =====
        if data == "adm_give_player_" or data == "adm_take_player_":
            if admin_id != user.id:
                await ctx.bot.send_message(chat_id=int(uid), text="Только админ.")
                return
        ...
        if data.startswith("adm_give_goods_"):
            if admin_id != user.id:
                await ctx.bot.send_message(chat_id=int(uid), text="Только админ.")
                return
            pid_str = data.replace("adm_give_goods_", "")
        ...
        if data.startswith("adm_take_goods_"):
            if admin_id != user.id:
                await ctx.bot.send_message(chat_id=int(uid), text="Только админ.")
                return
            pid_str = data.replace("adm_take_goods_", "")
        ...
        if data.startswith("adm_give_troops_"):
            if admin_id != user.id:
                await ctx.bot.send_message(chat_id=int(uid), text="Только админ.")
                return
            pid_str = data.replace("adm_give_troops_", "")
        ...
        if data.startswith("adm_take_troops_"):
            if admin_id != user.id:
                await ctx.bot.send_message(chat_id=int(uid), text="Только админ.")
                return
            pid_str = data.replace("adm_take_troops_", "")
        ...
        if data.startswith("adm_troops_"):
            if admin_id != user.id:
                await ctx.bot.send_message(chat_id=int(uid), text="Только админ.")
                return
            pid_str = data.replace("adm_troops_", "")
            player = bot_data.get("players", {}).get(pid_str)
            if not player:
                await ctx.bot.send_message(chat_id=int(uid), text="Игрок не найден.")
                return
        ...
        if data.startswith("adm_troop_add_"):
            if admin_id != user.id:
                await ctx.bot.send_message(chat_id=int(uid), text="Только админ.")
                return
            pid_str = data.replace("adm_troop_add_", "")
        ...
        # ===== ADMIN: STORAGE/BASE/FOCUS/ACTIONS =====
        ...
        if data.startswith("adm_send_turn_to_"):
            ...
            pid_str = data.replace("adm_send_turn_to_", "")
            player = bot_data.get("players", {}).get(pid_str)
            if not player:
                await ctx.bot.send_message(chat_id=int(uid), text="Игрок не найден.")
                return
        ...
        if data.startswith("adm_send_immediately_"):
            ...
            pid_str = data.replace("adm_send_immediately_", "")
        ...
        if data == "adm_production_":
            if admin_id != user.id:
                await ctx.bot.send_message(chat_id=int(uid), text="Только админ.")
                return
            pid_str = data.replace("adm_production_", "")
            player = players.get(pid_str)
            if not player:
                await ctx.bot.send_message(chat_id=int(uid), text="Игрок не найден.")
                return
        ...
        if data.startswith("adm_prod_remove_"):
            if admin_id != user.id:
                return
            pid_str = data.replace("adm_prod_remove_", "")
            player = players.get(pid_str)
            if not player:
                await ctx.bot.send_message(chat_id=int(uid), text="Игрок не найден.")
                return
        ...
        if data.startswith("adm_prod_edit_"):
            if admin_id != user.id:
                return
            pid_str = data.replace("adm_prod_edit_", "")
        ...
        if data == "adm_delete_player_":
            if admin_id != user.id:
                await ctx.bot.send_message(chat_id=int(uid), text="Только админ.")
                return
            pid_str = data.replace("adm_delete_player_", "")
            players_map = bot_data.get("players", {})
    ...
    try:
        ...
        if ctx.user_data.get("settlement_withdraw_amount"):
            try:
                ...
                idx = ctx.user_data.get("current_settlement_idx", 0)
                if idx < 0 or idx >= len(settlements):
                    await ctx.bot.send_message(chat_id=uid, text="Поселение не найдено.")
                    return
                settlement = settlements[idx]
                balance = settlement.get("balance", 0)
                if balance < amount:
        ...
        if ctx.user_data.get("adm_set_create") and uid == admin_id:
            ...
            save_data(ctx)
            await ctx.bot.send_message(chat_id=uid, text=f"✅ Поселение '{text}' создано для игрока {pid_str}.")
            try:
                await ctx.bot.send_message(chat_id=int(pid_str), text=f"✅ Админ создал поселение: '{text}'.")
            except:
                pass
            add_log(ctx, f"Админ {user.first_name} создал поселение '{text}' для игрока {pid_str}")
            return

        # ===== ADMIN: EDIT SETTLEMENT NAME =====
        if ctx.user_data.get("adm_set_edit_name") and uid == admin_id:
            pid_str, set_idx = ctx.user_data.pop("adm_set_edit_name")
            player = players.get(pid_str)
            if not player:
                await ctx.bot.send_message(chat_id=uid, text="Игрок не найден.")
                return
            ...
            if set_idx < 0 or set_idx >= len(settlements):
                ...
                return
            settlement = settlements[set_idx]
            old_name = settlement.get("name", "Поселение")
            settlement["name"] = text
            save_data(ctx)
            await ctx.bot.send_message(chat_id=uid, text=f"✅ Название поселения изменено с '{old_name}' на '{text}'")
            try:
        ...
        # ===== ADMIN: ADD SETTLEMENT POPULATION =====
        if ctx.user_data.get("adm_set_add_pop") and uid == admin_id:
            pid_str, set_idx = ctx.user_data.pop("adm_set_add_pop")
            try:
                qty = int(text)
                if qty <= 0:
                    await ctx.bot.send_message(chat_id=uid, text="Количество должно быть положительным.")
        ...
        # ===== ADMIN: REMOVE SETTLEMENT POPULATION =====
        if ctx.user_data.get("adm_set_remove_pop") and uid == admin_id:
            pid_str, set_idx = ctx.user_data.pop("adm_set_remove_pop")
            try:
                qty = int(text)
                if qty <= 0:
                    await ctx.bot.send_message(chat_id=uid, text="Количество должно быть положительным.")
        ...
        # ===== ADMIN: SET SETTLEMENT INCOME =====
        if ctx.user_data.get("adm_set_set_income") and uid == admin_id:
            pid_str, set_idx = ctx.user_data.pop("adm_set_set_income")
            try:
                income = int(text)
                player = players.get(pid_str)
                if not player:
        ...
        # ===== ADMIN: ADD SETTLEMENT BALANCE =====
        if ctx.user_data.get("adm_set_add_balance") and uid == admin_id:
            pid_str, set_idx = ctx.user_data.pop("adm_set_add_balance")
            try:
                amount = int(text)
                if amount <= 0:
                    await ctx.bot.send_message(chat_id=uid, text="Сумма должна быть положительной.")
                ...
                settlement["balance"] = settlement.get("balance", 0) + amount
                save_data(ctx)
                await ctx.bot.send_message(chat_id=uid, text=f"✅ На баланс поселения д��бавлено {amount} крышек. Новый баланс: {settlement['balance']}")
                try:
        ...
        # ===== ADMIN: REMOVE SETTLEMENT BALANCE =====
        if ctx.user_data.get("adm_set_remove_balance") and uid == admin_id:
            pid_str, set_idx = ctx.user_data.pop("adm_set_remove_balance")
            try:
                amount = int(text)
                if amount <= 0:
                    await ctx.bot.send_message(chat_id=uid, text="Сумма должна быть положительной.")
        ...
        # ===== ADMIN: SET SETTLEMENT TIME =====
        if ctx.user_data.get("adm_set_set_time") and uid == admin_id:
            pid_str, set_idx = ctx.user_data.pop("adm_set_set_time")
            try:
                ...
                if job_queue:
                    schedule_settlement_income(job_queue, pid_str, set_idx, dt_time(hour, minute))
        ...
        if ctx.user_data.get("awaiting_admin_add_item") and uid == admin_id:
            ...
            if not player:
                await ctx.bot.send_message(chat_id=uid, text="Игрок не найден.")
                return
            players[target].setdefault(typ, []).append(text)
        ...
        if ctx.user_data.get("awaiting_admin_amount") and uid == admin_id:
            ...
            if mode == "give":
                ...
                try:
                    await ctx.bot.send_message(chat_id=int(target),
                                               text=f"⚠️ У тебя отняли {amt} крышек (админ). Твой баланс: {players[target]['caps']}.")
                except:
                    pass
        ...
        if ctx.user_data.get("awaiting_admin_take_goods") and uid == admin_id:
            ...
            try:
                ...
                save_data(ctx)
                try:
                    await ctx.bot.send_message(chat_id=int(target), text=f"⚠️ Админ изъял у тебя {qty} товара.")
                except:
                    pass
            except ValueError:
                await ctx.bot.send_message(chat_id=uid, text="Введите целое число.")
        ...
        if ctx.user_data.get("awaiting_admin_take_troops") and uid == admin_id:
            ...
            try:
                ...
                save_data(ctx)
                try:
                    await ctx.bot.send_message(chat_id=int(target), text=f"⚠️ Админ изъял у тебя {qty} людей.")
                except:
                    pass
            except ValueError:
                await ctx.bot.send_message(chat_id=uid, text="Нужно ввести целое число.")
        ...
        logger.exception(f"Ошибка в text_handler: {e}")
        await ctx.bot.send_message(chat_id=uid, text=f"⚠️ Ошибка: {str(e)[:100]}")
        # ===== ERROR HANDLER =====
    ...
    try:
        if update.effective_user:
            await ctx.bot.send_message(
                ...
                text="⚠️ Произошла ошибка. Попробуй позже или свяжись с администратором."
            )
    except:
        pass
