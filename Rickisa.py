    ...
    async def cmd_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
            ...
            if data.startswith("trade_faction_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid), text="Выберите покупателя:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data.startswith("trade_buyer_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text="Введите количество товара для продажи (целое число):")
            ...
            if data.startswith("prod_select_"):
                ...
                if job_queue:
                    ...
                    logger.error(
                        "JobQueue отсутствует, производство для игрока {} не будет работать по расписанию.".format(uid))
            ...
            if data == "troop_add":
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text="Введи войско в формате: Название Кол-во (например: ГУЛЬ 50)")
                return
            ...
            if data == "troop_manage":
                ...
                for i, t in enumerate(troops, 1):
                    kb.append(
                        [InlineKeyboardButton(f"{i}. {t['name']} ({t['qty']})", callback_data=f"troop_item_{i - 1}")])
                kb.append([InlineKeyboardButton("Назад", callback_data="player_troops")])
                await ctx.bot.send_message(chat_id=int(uid), text="Выбери войско:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data.startswith("troop_edit_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text="Введи новое значение: Название Кол-во (пример: ГУЛЬ 100)")
                return
            ...
            if data == "storage_manage":
                ...
                await ctx.bot.send_message(chat_id=int(uid), text="Выбери запись склада:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data == "base_manage":
                ...
                await ctx.bot.send_message(chat_id=int(uid), text="Выбери запись базы:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data == "focus_manage":
                ...
                await ctx.bot.send_message(chat_id=int(uid), text="Выбери запись фокуса:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data == "actions_manage":
                ...
                await ctx.bot.send_message(chat_id=int(uid), text="Выбери запись:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data == "settlement_withdraw":
                ...
                if balance <= 0:
                    await ctx.bot.send_message(chat_id=int(uid),
                                               text=f"На поселении нет крышек для снятия. Баланс: {balance}")
                    return
            ...
            if data == "player_profile":
                ...
                await ctx.bot.send_message(chat_id=int(uid), text=txt,
                                           reply_markup=player_main_kb(bot_data.get("ADMIN_ID") == user.id))
                return
            ...
            if data == "player_transfer":
                ...
                kb.append([InlineKeyboardButton("Назад", callback_data="player_profile")])
                await ctx.bot.send_message(chat_id=int(uid), text="Выберите фракцию получателя:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data.startswith("transfer_faction_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid), text="Выберите получателя:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data == "diplo_info":
                ...
                for tid, t in my_treaties:
                    ...
                    if t.get("pending"):
                        ...
                        text += f"• {type_name} с {other_faction}\n  до {end_str}\n\n"
                    kb.append([InlineKeyboardButton(f"Расторгнуть {type_name} с {other_faction}",
                                                    callback_data=f"diplo_cancel_{tid}")])
            ...
            if data == "diplo_new":
                ...
                kb.append([InlineKeyboardButton("Назад", callback_data="player_diplomacy")])
                await ctx.bot.send_message(chat_id=int(uid), text="Выберите фракцию для предложения:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data in ("diplo_pact", "diplo_war", "diplo_military", "diplo_trade"):
                ...
                if treaty_type == "war":
                    ...
                    add_log(ctx,
                            f"Игрок {from_name} ({uid}) объявил войну фракции {target_faction} (начнётся через {duration} дн.)")
                    if admin_id:
                        try:
                            await ctx.bot.send_message(
                                chat_id=int(admin_id),
                    ...
                    add_log(ctx,
                            f"Игрок {players[uid].get('faction')} ({uid}) предложил {treaty_type} фракции {target_faction}")
                return
                ...
                try:
                    ...
                    if current_gov:
                        ...
                        if time.time() - last_change < 2 * 24 * 3600:
                            await ctx.bot.send_message(chat_id=int(uid),
                                                       text="Вы можете менять форму правления не чаще раза в 2 дня.")
                            return
                    ...
                    if gov_type == "рабский_труд":
                        ...
                        if not people_troop or people_troop["qty"] < 10:
                            await ctx.bot.send_message(chat_id=int(uid),
                                                       text="Недостаточно людей для рабского труда (нужно минимум 10).")
                            return
                        people_troop["qty"] -= 10
                        if people_troop["qty"] <= 0:
                            troops.remove(people_troop)
            ...
            if data == "adm_players":
                ...
                await ctx.bot.send_message(chat_id=int(uid), text="Выбери игрока:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data.startswith("adm_give_goods_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text=f"Введите количество товара для выдачи игроку {pid_str}:")
                return
            ...
            if data.startswith("adm_take_goods_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text=f"Введите количество товара для изъятия у игрока {pid_str}:")
                return
            ...
            if data.startswith("adm_prod_remove_"):
                ...
                try:
                    await ctx.bot.send_message(chat_id=int(pid_str),
                                               text="Админ снял ваше производство. Люди возвращены.")
                except:
                    pass
                return
            ...
            if data.startswith("adm_set_manage_"):
                ...
                kb = [
                    [InlineKeyboardButton("✏️ Изменить название",
                                          callback_data=f"adm_set_edit_name_{pid_str}_{set_idx}")],
                    [InlineKeyboardButton("👥 Добавить население",
                                          callback_data=f"adm_set_add_pop_{pid_str}_{set_idx}")],
                    [InlineKeyboardButton("👥 Забрать население",
                                          callback_data=f"adm_set_remove_pop_{pid_str}_{set_idx}")],
                    [InlineKeyboardButton("💰 Установить доход",
                                          callback_data=f"adm_set_set_income_{pid_str}_{set_idx}")],
                    [InlineKeyboardButton("➕ Добавить крышки",
                                          callback_data=f"adm_set_add_balance_{pid_str}_{set_idx}")],
                    [InlineKeyboardButton("➖ Забрать крышки",
                                          callback_data=f"adm_set_remove_balance_{pid_str}_{set_idx}")],
                    [InlineKeyboardButton("⏰ Установить время дохода",
                                          callback_data=f"adm_set_set_time_{pid_str}_{set_idx}")],
            ...
            if data.startswith("adm_set_delete_"):
                ...
                try:
                    await ctx.bot.send_message(chat_id=int(pid_str),
                                               text=f"❌ Админ удалил ваше поселение '{removed.get('name')}'.")
                except:
                    pass
            ...
            if data.startswith("adm_set_set_income_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text="Введите ежедневный доход/налог (может быть отрицательным, например: -50):")
                return
            ...
            if data.startswith("adm_set_add_balance_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text="Введите количество крышек для добавления на баланс поселения:")
                return
            ...
            if data.startswith("adm_set_remove_balance_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text="Введите количество крышек для вычитания из баланса поселения:")
                return
            ...
            if data.startswith("adm_set_set_time_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text="Введите время для ежедневного дохода в формате ЧЧ:ММ (например: 09:00):")
                return
            ...
            if data.startswith("adm_government_"):
                ...
                if gov:
                    await ctx.bot.send_message(chat_id=int(uid),
                                               text=f"У игрока уже есть форма правления: {gov['type']}. Чтобы изменить, используйте сброс через редактирование игрока.")
                else:
                    await ctx.bot.send_message(chat_id=int(uid),
                                               text="У игрока нет формы правления. Вы можете назначить её через редактирование.")
                return
            ...
            if data == "adm_all_troops":
                ...
                await ctx.bot.send_message(chat_id=int(uid), text="Выбери действие:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data == "adm_set_production_time":
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text="Введите новое время для производства в формате ЧЧ:ММ (например, 18:00):")
                return
            ...
            if data == "adm_set_government_time":
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text="Введите новое время для формы правления в формате ЧЧ:ММ (например, 18:00):")
                return
            ...
            if data == "adm_diplo_set_pact":
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text="Введите новую длительность пакта о ненападении (в днях):")
                return
            ...
            if data == "adm_diplo_set_war":
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text="Введите новую длительность до начала войны (в днях):")
                return
            ...
            if data == "adm_diplo_set_trade":
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text="Введите новую длительность торгового союза (в днях):")
                return
            ...
            if data == "adm_government":
                ...
                kb = [
                    ...
                    [InlineKeyboardButton("Изменить людские отношения",
                                          callback_data="adm_gov_edit_людские_отношения")],
                    [InlineKeyboardButton("Изменить трудовые отношения",
                                          callback_data="adm_gov_edit_трудовые_отношения")],
                    [InlineKeyboardButton("Назад", callback_data="admin_open")]
                ]
                await ctx.bot.send_message(chat_id=int(uid), text=text, reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data.startswith("adm_gov_edit_"):
                ...
                if gov_type == "рабский_труд":
                    await ctx.bot.send_message(chat_id=int(uid),
                                               text="Введите новые параметры для рабского труда в формате: крышек_в_день (например: 40)")
                elif gov_type == "людские_отношения":
                    await ctx.bot.send_message(chat_id=int(uid),
                                               text="Введите новые параметры для людских отношений в формате: людей_в_день стоимость_крышек (например: 3 30)")
                elif gov_type == "трудовые_отношения":
                    await ctx.bot.send_message(chat_id=int(uid),
                                               text="Введите новые параметры для трудовых отношений в формате: товара_в_день стоимость_крышек (например: 15 15)")
                return
            ...
            if data.startswith("adm_troop_del_"):
                ...
                if 0 <= idx < len(troops):
                    ...
                    try:
                        await ctx.bot.send_message(chat_id=int(pid_str),
                                                   text=f"Админ удалил твоё войско: {removed['name']}")
                    except:
                        pass
                else:
                    await ctx.bot.send_message(chat_id=int(uid), text="Нет такого войска.")
            ...
            if data.startswith("adm_storage_") or data.startswith("adm_base_") or data.startswith(
                    "adm_focus_") or data.startswith("adm_actions_"):
                if admin_id != user.id:
                    await ctx.bot.send_message(chat_id=int(uid), text="Только админ.")
                    return
                parts = data.split("_")
                if len(parts) < 3:
                    ...
                    return
                typ = parts[1]
                if parts[2].startswith("item_") or parts[2].startswith("add_") or parts[2].startswith("edit_") or parts[
                    2].startswith("del_"):
                    # Обработка подменю
                    if parts[2] == "add_":
                        ...
                        await ctx.bot.send_message(chat_id=int(uid), text=f"{item}",
                                                   reply_markup=InlineKeyboardMarkup(kb))
                        return
            ...
            if data == "adm_shop":
                ...
                for i, it in enumerate(shop, 1):
                    kb.append(
                        [InlineKeyboardButton(f"{i}. {it['title']} ({it['price']})",
                                              callback_data=f"adm_shop_item_{i - 1}")])
                kb.append([InlineKeyboardButton("Назад", callback_data="admin_open")])
                await ctx.bot.send_message(chat_id=int(uid), text="Выбери товар:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data and data.startswith("adm_logs_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid), text=text_block,
            ...
            if data.startswith("adm_sched_pay_player_"):
                ...
                kb = [
                    ...
                    [InlineKeyboardButton("Отменить выплату товара",
                                          callback_data=f"adm_sched_cancel_goods_{pid_str}")],
                    [InlineKeyboardButton("Назад", callback_data="adm_scheduled_payments")]
                ]
                await ctx.bot.send_message(chat_id=int(uid), text="Выберите тип выплаты:",
                                           reply_markup=InlineKeyboardMarkup(kb))
                return
            ...
            if data.startswith("adm_sched_cancel_caps_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text=f"Плановая выплата крышек для игрока {pid_str} отменена.")
                try:
                    await ctx.bot.send_message(chat_id=int(pid_str),
                                               text="Ваша плановая выплата крышек отменена администратором.")
                except:
                    pass
                return
            ...
            if data.startswith("adm_sched_cancel_goods_"):
                ...
                await ctx.bot.send_message(chat_id=int(uid),
                                           text=f"Плановая выплата товара для игрока {pid_str} отменена.")
                try:
                    await ctx.bot.send_message(chat_id=int(pid_str),
                                               text="Ваша плановая выплата товара отменена администратором.")
                except:
                    pass
                return
            ...
            def main():
                ...
                if app.job_queue is not None:
                    ...
                    logger.warning(
                        "JobQueue не настроен. Производство, форма правления и войны по расписанию работать не будут.")
